...
class SaveCampaignsOfAdonator(APIView):
    ...
    def getLatLngFromGoogle(self, address):
        api_response = requests.get('https://maps.googleapis.com/maps/api/geocode/json?address={0}&key={1}'.format(address, settings.GOOGLE_MAPS_API_KEY))
    ...
    def savePhoto(self, photoData, campaignId):
        photoCampaign = CampaignPhoto.objects.filter(campaign_id=campaignId).first()

        if photoCampaign:
            photo = Photo.objects.filter(pk=photoCampaign.photo_id).first()
        else: 
            photo = Photo.objects.create(photo="")
            CampaignPhoto.objects.create(campaign_id=campaignId, photo_id=photo.id)
    ...
    def put(self, request):
        ...
        if photoData:
            ...
            photo.photo.save(photoData["img_name"], decodedImg)
            CampaignPhoto.objects.create(photo_id=photo.id, campaign_id=campaign.id)
...
class filterCampaign(APIView):
    def get(self, request):
        ...
        if campaignsWithTagId:
            if campaignName:
                queryset = Campaign.objects.filter(id__in=campaignsWithTagId, name__contains=campaignName)
            else:
